...
var step = (row) => {
  ...
  var partyName = vote.partido;

  if (partyName) {
    if (parties[partyName] == null) {
      parties[partyName] = {
    ...
    Object.keys(pCutoffs).forEach((period) => {
      if (fecha >= pCutoffs[period][0] && fecha < pCutoffs[period][1]) {
        if (vote.voto == "No") {
          parties[partyName][period].no++;
          parties[partyName][period].total++;
        } else if (vote.voto == "Si") {
          parties[partyName][period].si++;
          parties[partyName][period].total++;
        } else if (vote.voto == "Sin registro") {
          parties[partyName][period].sr++;
          parties[partyName][period].total++;
        }
      }
    });
